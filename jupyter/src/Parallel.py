...
def presentation_thread_classify_function(file_lock, results_lock, knn, test_data, total_correct, total_wrong, metric=euclidean_distance):
    total_start = time.time()
    console_log = "Alguns resultados:"
    log = ""
    count = 0
    correct = 0
    wrong = 0
    for point, right_answer in test_data:
        ...
        log += f"{right_answer},{returned_answer},{returned_answer == right_answer},{total}\n"
        if count < 3:
            console_log += f"\nCerto: {right_answer}; Respondido: {returned_answer}; Acertou: {returned_answer == right_answer}; Tempo de processamento: {total}s"
        count += 1
    ...
    print(f"\nTempo de execução {multiprocessing.current_process().name}: {time.time() - total_start}s\n")